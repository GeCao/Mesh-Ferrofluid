...
class AbstractHyperSingularLayer(ABC):
    ...
    def __init__(
        ...
        self.unpacked_D_mat = torch.Tensor([[[]]]).to(self.device).to(self.dtype)
    ...
    def get_Dmat_unpacked(self) -> torch.Tensor:
        return self.unpacked_D_mat  # [N_faces, N_phiy, N_faces, N_phix]

    def get_Dmat_tight(self, verts: torch.Tensor, faces: torch.Tensor) -> torch.Tensor:
        """Get The D_mat in tight format
        Our D_mat is saved with the format of: [N_faces, N_phiy, N_faces, N_phix]
        ...
        return pack_matrix_tight(
            ...
            unpacked_matrix=self.unpacked_D_mat,
            verts=verts,
            faces=faces,
        )
