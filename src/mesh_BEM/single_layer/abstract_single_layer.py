...
class AbstractSingleLayer(ABC):
    ...
    def __init__(
        ...
        self.unpacked_V_mat = torch.Tensor([[[]]]).to(self.device).to(self.dtype)
    ...
    def get_Vmat_unpacked(self) -> torch.Tensor:
        return self.unpacked_V_mat  # [N_faces, N_phiy, N_faces, N_phix]

    def get_Vmat_tight(self, verts: torch.Tensor, faces: torch.Tensor) -> torch.Tensor:
        """Get The V_mat in tight format
        Our V_mat is saved with the format of: [N_faces, N_phiy, N_faces, N_phix]
        ...
        return pack_matrix_tight(
            ...
            unpacked_matrix=self.unpacked_V_mat,
            verts=verts,
            faces=faces,
        )
